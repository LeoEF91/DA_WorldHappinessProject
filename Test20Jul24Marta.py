...
st.markdown("<h1 style='text-align: center;'>World Happiness Project</h1>", unsafe_allow_html=True)
st.sidebar.title("Table of contents")
pages = ["Overview", "Data exploration", "Data Viz","Data Pre-processing" , "Machine Learning", "Predictions countries", "Summary"," References" ]
page = st.sidebar.radio("Go to", pages)

# Load the datasets
@st.cache_data
def load_data():
    df = pd.read_csv(r"C:\Users\HP\Desktop\Presentation\merged_df_only_common_columns2.csv")
    return df
...
if page == pages[0]: 
    st.write("### Overview")
    st.image(r"C:\Users\HP\Desktop\Presentation\WHR1.jpg", caption='WHR.', use_column_width=True)
...
Plotly map 2021[MB1], interactive. Title : Figure 1: Ladder score distribution across countries in 2021 (#5.1. section, )
...
Regions graph. Title: Figure 2: Boxplot with Ladder score distributions by Region (2021) (#5.4.)
...
Scatterplot with Ladder Score, but the user can pick which dimension it combines it with (covering #5.2. and #5.7.). Title: Figure 3: Scatter plot with Ladder score.

To offer a better overview with exact numbers, we also created a Heatmap:   
Heatmap: Title: Figure 5: Heatmap (2005-2021) (#5.6.)
...
Title: Figure 6: The happiest and least happy countries according to the average values of Ladder score (between 2005-2021) (#5.5.)
""")
...
if page == pages[4]: 
    ...
    st.image(r"C:\Users\HP\Desktop\Presentation\Table performance score_black background.jpg", caption='Comparison of performace scores for three models', use_column_width=True)
...
""")
    ...
    df['Predicted Ladder score'] = predictions
    
    top_5_happiest = df.nlargest(5, 'Predicted Ladder score')[['Country name', 'Predicted Ladder score']]
    bottom_5_unhappiest = df.nsmallest(5, 'Predicted Ladder score')[['Country name', 'Predicted Ladder score']]
    
    st.write("#### Top 5 Predicted Happiest Countries for 2022")
    st.table(top_5_happiest)
    
    st.write("#### Bottom 5 Predicted Unhappiest Countries for 2022")
    st.table(bottom_5_unhappiest)
